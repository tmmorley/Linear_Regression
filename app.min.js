Alteryx.Gui.BeforeLoad = function(manager, AlteryxDataItems){
  ...
  dataItem('standardize_pred', {value: true}, 'SimpleBool')
  dataItem('internal_cv', {value: true}, 'SimpleBool')
  dataItem('display_graphs', {value: true}, 'SimpleBool')
  dataItem('set_seed_internal_cv', {value: true}, 'SimpleBool')
  dataItem('set_seed_external_cv', {value: true}, 'SimpleBool')
  dataItem('Model Name', {value: getDefaultModelName()})
  var radioItems = [
    ['lambda_min', 'lambda_1se']
  ]
  radioItems.forEach(function(d){
...
Alteryx.Gui.AfterLoad = function(manager){
  ...
  //handleSetupComplete();
  hideRegularizationOnXDF(manager);
  ...
  displayTarget('show-on-regularization-graph', 'regularization')
  ...
  var xvars = manager.GetDataItemByDataName('X Vars')
  /*
  if (xvars.value && xvars.value.length <= 1){
    var v2 = xvars.value[0].split(" + ")
    console.log(v2)
    xvars.setValue(v2)
  }
  */
